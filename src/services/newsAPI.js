...
const getBreakingNews = (date) => {
  return fetchRequest(
    `top-headlines?category=sports&q=football&from=${moment().format(
      'YYYY-MM-DD'
    )}&apiKey=${API_KEY}`
  )
    ...
    .then((articles) =>
      articles.filter(
        (article) =>
          !article.title.includes('NFL') &&
          !article.title.includes('quarterbacks') &&
          !article.url.includes('www.sportingnews.com') &&
          !article.title.includes('Frank')
      )
    );
};
...
const getTeamNews = (teamName) => {
  return fetchRequest(
    `everything?q=${teamName}&from=${moment()
      .subtract(3, 'days')
      .format('YYYY-MM-DD')}&to=${moment().format('YYYY-MM-DD')}&sortBy=relevancy&apiKey=${API_KEY}`
  ).then((data) => data.articles.filter((article) => article.title.includes(teamName)));
};

const getLeagueNews = (leagueName) => {
  return fetchRequest(
    `everything?q=${leagueName}&from=${moment()
      .subtract(3, 'days')
      .format('YYYY-MM-DD')}&to=${moment().format('YYYY-MM-DD')}&sortBy=relevancy&apiKey=${API_KEY}`
