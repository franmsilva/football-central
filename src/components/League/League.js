...
const League = () => {
  ...
  useEffect(() => {
    ...
    footballAPI.getLeagueStandings(leagueID).then((standings) => setLeagueStandings(standings));
    footballAPI
      .getTopScorers(leagueID)
      .then((topScorers) => setTopScorers(topScorers.slice(0, 10)));
    newsAPI.getLeagueNews(leagueName).then((news) => setLeagueNews(news));
    footballAPI.getCurrentRound(leagueID).then((round) => setCurrentRound(round));
    footballAPI
        ...
        <React.Fragment>
          <div className="league__header">
            <div className="league__logo">
              <Image alt="" src={leagueFixtures[0].league.logo} width={80} />
            </div>
